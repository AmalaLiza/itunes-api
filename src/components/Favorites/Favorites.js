        ...
        <div>
          ...
          <ul className={styles.list}>
            ...
            {favorites
              ...
              .map((album) => <li className={styles.listItem}>
                <div className={styles.listItemImage}
                     style={{ backgroundImage: `url("${album.get('artworkUrl100')}")` }}>
                </div>
                <div
                  className={classNames(styles.listItemTitle, styles['text-ellipsis'])}>{album.get('artistName')}</div>
                <div
                  className={classNames(styles.listItemSinger, styles['text-ellipsis'])}>{album.get('trackName')}</div>
              </li>)}
          </ul>
        </div>
        <div className={styles.artistWrapper}>
          <h2 className={styles.heading}>ARTISTS</h2>
          {artists && artists.size > 0 && <Artists artists={artists} onClick={artistOnClick} />}
        </div>
      </div>
    );
  }
